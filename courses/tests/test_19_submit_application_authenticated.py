...
class SubmitApplicationAuthenticatedTest(ManagedModelTestCase):
    ...
    def test_submit_application_success(self):
        ...
        self.assertEqual(data['message'], 'Заявка успешно отправлена')
        
        new_count = Applications.objects.count()
        self.assertEqual(new_count, old_count + 1)
        
        application = Applications.objects.filter(user=self.user, course=self.course).first()
